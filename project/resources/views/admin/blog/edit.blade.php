        ...
        <div class="card mb-4">
            ...
            <div class="card-body">
                ...
                <form action="{{route('admin.blog.update',$blog->id)}}" enctype="multipart/form-data" method="POST">
                        ...
                        <div class="col-md-6 ">
                            <div class="form-group">
                                ...
                                <select class="form-control  mb-3" id="categorys" name="category_id" required>
                                    ...
                                    <option value="{{$item->id}}" {{$blog->category_id == $item->id ?
                    ...
                    <div class="form-group">
                        <label for="sort_text">{{ __('Sort Description') }} {{$blog->sort_text}}</label>
                        <textarea id="sort_text" class="form-control" name="sort_text"
                            placeholder="{{ __('Sort Description') }}" required>{{$blog->sort_text}}</textarea>
                    </div>

                    <div class="form-group">
                        <label for="description">{{ __('Description') }}</label>
                        <textarea id="area1" class="form-control summernote" name="description"
                            placeholder="{{ __('Description') }}" required>{{$blog->description}}</textarea>
                    </div>
