                ...
                <div class="card-body">
                    ...
                    <form action="{{ route('admin.blog.update', $blog->id) }}" enctype="multipart/form-data" method="POST">
                                    ...
                                    <select class="form-control  mb-3" id="categorys" name="category_id" required>
                                        @foreach ($categories as $item)
                                            <option value="{{ $item->id }}"
                                                {{ $blog->category_id == $item->id ? 'selected' : '' }}>
                                                {{ $item->name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                        ...
                        <div class="form-group">
                            ...
                            <textarea id="sort_text" class="form-control" name="sort_text" placeholder="{{ __('Sort Description') }}" required>{{ $blog->sort_text }}</textarea>
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control  mb-3" name="status" required>
                                <option value="1" {{ $blog->status == 1 ? 'selected' : '' }}>{{ __('Active') }}
                                </option>
                                <option value="0" {{ $blog->status == 0 ? 'selected' : '' }}>{{ __('Inactive') }}
                                </option>
                            </select>
                        </div>
